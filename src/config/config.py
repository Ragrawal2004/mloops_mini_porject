...
DAGSHUB_REPO_NAME: str = "mloops_mini_porject"
...
def configure_mlflow_tracking() -> None:
    ...
    Reads the DagsHub token from the environment, sets the MLflow basic-auth
    environment variables, and points MLflow's tracking URI at the DagsHub
    MLflow server. Centralizing this avoids duplicating the same four lines
    in ``model_evaluation.py``, ``register_model.py``, ``promote_model.py``,
